...
class Dashboard extends Controller
{
    ...
    public function do_update_slider_bukutamu(){
        ...
        $slider1 = $this->request->getFile('foto_slider1');
        $slider2 = $this->request->getFile('foto_slider2');
        $slider3 = $this->request->getFile('foto_slider3');
        $kunci = $this->request->getPost('kunci');
        $path = 'assets/users/'.$kunci;
        ...
        if($slider1 != ''){ //cek dulu ini fotonya siapa
        	$avatar = $slider1;
        	$pathName = 'assets/users/'.$kunci.'/slider1.png';
        	if(file_exists($pathName)){
        		unlink($pathName); //hapus dulu foto yg lama
	    	}   
	    	    if(!file_exists($pathName)){
                ...
                $saveSlider = $this->DashboardModel->save_slider($dataSlider);
				$avatar->move('assets/users/'.$kunci, 'slider1.png'); //upload yg baru
				$this->session->set('foto_slider1', 1);
				
				echo 'uploadedslider1'; //give feedback ke jquery.. agar tampilan fotonya di ubah dgn yg baru
				
        }else if($slider2 != ''){
            $avatar = $slider2;
            $pathName = 'assets/users/'.$kunci.'/slider2.png';
            if(file_exists($pathName)){
                unlink($pathName);
            } 
            if(!file_exists($pathName)){
            ...
            $avatar->move('assets/users/'.$kunci, 'slider2.png');
            $this->session->set('foto_slider2', 1);
            
            echo 'uploadedslider2';
            
        }else{
            $avatar = $slider3;
            $pathName = 'assets/users/'.$kunci.'/slider3.png';
            if(file_exists($pathName)){
                unlink($pathName);
            } 
            if(!file_exists($pathName)){
            ...
            $avatar->move('assets/users/'.$kunci, 'slider3.png');
           
            $this->session->set('foto_slider3', 1);
            
            echo 'uploadedslider3';
        } 	
     ...
     public function data_hadir(){
        ...
        //$data['tamu'] = $this->DashboardModel->get_tamu_by_qrcode();
        $data['data'] = $this->DashboardModel->get_data_by_id_user();
